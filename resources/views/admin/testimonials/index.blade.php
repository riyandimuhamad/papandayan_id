...
<div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    <div>
        <h2 class="text-xl font-bold text-gray-800">Daftar Testimoni</h2>
        <p class="text-sm text-gray-500">Kelola ulasan dan pengalaman klien papandayan_id.</p>
    </div>
    <a href="{{ route('admin.testimonials.create') }}" class="inline-flex items-center px-4 py-2 bg-slate-900 border border-transparent rounded-lg font-semibold text-xs text-white uppercase tracking-widest hover:bg-slate-800 focus:bg-slate-800 active:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2 transition ease-in-out duration-150 shadow-sm">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Paket & Rating</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Pesan</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <div class="flex items-center">
                            <div class="flex-shrink-0 h-10 w-10">
                                @if($testimonial->avatar_path)
                                    <img class="h-10 w-10 rounded-full object-cover border border-gray-200" src="{{ url('storage/' . $testimonial->avatar_path) }}" alt="{{ $testimonial->customer_name }}">
                                @else
                            ...
                            <div class="ml-4">
                                <div class="text-sm font-semibold text-gray-900">{{ $testimonial->customer_name }}</div>
                                <div class="text-xs text-gray-500">{{ $testimonial->trip_date ? \Carbon\Carbon::parse($testimonial->trip_date)->format('d M Y') : 'Trip Date N/A' }}</div>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-700 mb-1">
                            {{ $testimonial->package ? $testimonial->package->name : 'General Trip' }}
                        </div>
                        <div class="flex items-center">
                            @for($i=1; $i<=5; $i++)
                    ...
                    <td class="px-6 py-4">
                        <p class="text-sm text-gray-600 line-clamp-2 italic">"{{ $testimonial->message }}"</p>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div class="flex justify-end gap-2">
                            <a href="{{ route('admin.testimonials.edit', $testimonial) }}" class="text-blue-600 hover:text-blue-900 bg-blue-50 hover:bg-blue-100 p-2 rounded-md transition-colors" title="Edit">
                ...
                <tr>
                    <td colspan="4" class="px-6 py-10 text-center text-gray-500">
                        ...
                        <p class="mt-2 text-sm font-medium">Belum ada data testimoni.</p>
                        <p class="mt-1 text-sm text-gray-400">Mulai dengan menambahkan testimoni dari klien.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
