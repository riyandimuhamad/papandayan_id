...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="p-6 sm:p-8 border-b border-gray-100">
        <h3 class="text-lg font-bold text-gray-900">Form Edit Artikel</h3>
        <p class="text-sm text-gray-500">Ubah konten artikel blog Anda.</p>
    </div>
    
    <div class="p-6 sm:p-8">
        <form id="data-form" action="{{ route('admin.articles.update', $article) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="flex items-start gap-6">
                    <div class="w-48 h-32 rounded-lg bg-gray-100 border-2 border-dashed border-gray-300 flex items-center justify-center overflow-hidden" id="photo-preview-container">
                        @if($article->thumbnail_path)
                            <img id="photo-preview" class="w-full h-full object-cover" src="{{ url('storage/' . $article->thumbnail_path) }}" alt="Preview">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="mt-2 text-xs text-gray-500">Format yang diizinkan: JPG, PNG, GIF. Maksimal ukuran 5MB. Biarkan kosong jika tidak mengubah foto.</p>
                        @error('thumbnail')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 border-t border-gray-100 pt-6">
                ...
                <div class="md:col-span-2 space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $article->title) }}" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring focus:ring-yellow-500 focus:ring-opacity-50 text-lg">
                        @error('title')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="space-y-6 bg-gray-50 p-4 rounded-lg border border-gray-200 h-fit">
                    ...
                    <div>
                        ...
                        <input type="text" name="category" id="category" value="{{ old('category', $article->category) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring focus:ring-yellow-500 focus:ring-opacity-50">
                        @error('category')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="pt-4 border-t border-gray-200">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title', $article->meta_title) }}" class="w-full rounded-md border-gray-300 shadow-sm text-sm focus:border-yellow-500 focus:ring focus:ring-yellow-500 focus:ring-opacity-50">
                        </div>
                        
                        <!-- Meta Description -->
                        <div class="mb-3">
                            <label for="meta_description" class="block text-xs text-gray-600 mb-1">Meta Description</label>
                            <textarea name="meta_description" id="meta_description" rows="2" class="w-full rounded-md border-gray-300 shadow-sm text-sm focus:border-yellow-500 focus:ring focus:ring-yellow-500 focus:ring-opacity-50">{{ old('meta_description', $article->meta_description) }}</textarea>
                        </div>

                        <!-- Meta Keywords -->
                        <div>
                            <label for="meta_keywords" class="block text-xs text-gray-600 mb-1">Keywords</label>
                            <input type="text" name="meta_keywords" id="meta_keywords" value="{{ old('meta_keywords', $article->meta_keywords) }}" class="w-full rounded-md border-gray-300 shadow-sm text-sm focus:border-yellow-500 focus:ring focus:ring-yellow-500 focus:ring-opacity-50">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <input type="number" name="order" id="order" value="{{ old('order', $article->order ?? 0) }}" required min="0" class="w-24 rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring focus:ring-yellow-500 focus:ring-opacity-50">
            ...
            <div class="flex items-center justify-end pt-4 border-t border-gray-100 gap-3">
                <a href="{{ route('admin.articles.index') }}" class="px-5 py-2.5 bg-white border border-gray-300 rounded-lg font-bold text-gray-700 text-sm hover:bg-gray-50 focus:outline-none transition-all shadow-sm">Batal</a>
        ...
        ClassicEditor
            ...
            .then(editor => {
                editor.model.document.on('change:data', () => {
                    document.querySelector('.tinymce-editor').value = editor.getData();
                    window.checkChanges();
                });
            })
            .catch( error => {
