...
@section('header_title', 'Edit Slider')
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden max-w-4xl">
    <div class="p-6 sm:p-8 border-b border-gray-100">
        <h3 class="text-lg font-bold text-gray-900">Form Edit Slider</h3>
        <p class="text-sm text-gray-500">Ubah gambar dan caption untuk slider halaman utama.</p>
    </div>
    
    <div class="p-6 sm:p-8">
        <form id="slide-form" action="{{ route('admin.hero-slides.update', $heroSlide) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="flex items-center gap-6">
                    <div class="w-48 h-32 rounded-lg bg-gray-100 border-2 border-gray-300 flex items-center justify-center overflow-hidden" id="photo-preview-container">
                        <img id="photo-preview" class="w-full h-full object-cover" src="{{ strpos($heroSlide->image, 'http') === 0 ? $heroSlide->image : asset($heroSlide->image) }}" alt="Preview">
                    ...
                    <div class="flex-1">
                        <input type="file" name="image" id="image" accept="image/*" onchange="previewImage(event)" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-yellow-50 file:text-yellow-700 hover:file:bg-yellow-100 transition-colors">
                        <p class="mt-2 text-xs text-gray-500">Maksimal 2MB. Biarkan kosong jika tidak ingin mengubah gambar.</p>
                        @error('image')
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $heroSlide->title) }}" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring focus:ring-yellow-500 focus:ring-opacity-50">
                @error('title')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="subtitle" id="subtitle" rows="3" class="w-full rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring focus:ring-yellow-500 focus:ring-opacity-50">{{ old('subtitle', $heroSlide->subtitle) }}</textarea>
                @error('subtitle')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="number" name="order" id="order" value="{{ old('order', $heroSlide->order) }}" required min="0" class="w-24 rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring focus:ring-yellow-500 focus:ring-opacity-50">
                ...
                <div class="flex items-center">
                    ...
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $heroSlide->is_active) ? 'checked' : '' }} class="w-5 h-5 rounded border-gray-300 text-yellow-500 shadow-sm focus:ring-yellow-500">
